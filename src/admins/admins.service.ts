...
export class AdminsService {
  ...
  async create(createAdminDto: Prisma.AdminsCreateInput) {
    ...
    if (!email || !fullname || !password)
      return CustomErrorHandler('Fields are required', 'FORBIDDEN');
    let saltOrRounds = 10;
    ...
    try {
      ...
      if (existingAdmin)
        return CustomErrorHandler('User is already Exist', 'FORBIDDEN');
  ...
  async update(id: number, updateAdminDto: Prisma.AdminsUpdateInput) {
    try {
      ...
      if (!existingAdmin)
        return CustomErrorHandler('No admin found', 'INTERNAL_SERVER_ERROR');

      let updated_admin = await this.prisma.admins.update({
        where: { id: id },
        data: {
          ...withoutPassword,
          password: hash ? hash : existingAdmin.password,
        },
      });
  ...
  async adminlogin(login_credentials: loginAdminDto, res: Response) {
    try {
      ...
      if (!email || !password)
        return CustomErrorHandler('All fields are required', 'FORBIDDEN');

      let admin_user = await this.prisma.admins.findUnique({
        where: { email: email },
      });
      ...
      if (!passowrd_check)
        return CustomErrorHandler('Passowrd is incorrect', 'FORBIDDEN');
      let access_token = jwt.sign(payload, process.env.JWT_SECRET, {
        expiresIn: '24h',
      });
  ...
  async super_login_admin_handler(
    super_admin_login: Super_admin_dto,
    res: Response,
  ) {
    try {
      ...
      if (
        ...
        password !== process.env.super_admin_passoword
      ) {
  ...
  async getLast7DaysStats() {
    ...
    const dailyAppointmentsMap = appointments.reduce(
      (acc, appointment) => {
        const key = new Date(appointment?.createdAt ?? '')
          .toISOString()
          .split('T')[0];
