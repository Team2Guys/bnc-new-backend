...
export class ProductsService {
    ...
    AddProductHandler = async (createCategoryDto: Prisma.productsCreateInput, req: Request | any) => {
        ...
        const {recalledByCategories:recallCat,...withoutRecall } = createCategoryDto;

        const recalledByCategories = req.body.recalledByCategories; // assuming this is an array of IDs


        let AlreadyExistedProduct = await this.prisma.products.findUnique({ where: { title: withoutRecall.title } })
        ...
        try {
            ...
            if (recalledByCategories.length > 0) {
                data.recalledByCategories = {
                    connect: recalledByCategories.map((id: number) => ({ id: Number(id) })),
                };
            }
    ...
    UpdateProductHandler = async (id: number, updateProduct: Prisma.productsUpdateInput, req: Request | any) => {
        try {
            ...
            let updated_products = await this.prisma.products.update({
                where: { id: id },
                data: {
                    ...updateProduct, last_editedBy: email,

                    recalledByCategories: {
                        set: Array.isArray(recalledByCategories)
                            ? recalledByCategories.map((categoryId: number) => ({ id: Number(categoryId) }))
                            : [],
                    },
                    updatedAt
                },
